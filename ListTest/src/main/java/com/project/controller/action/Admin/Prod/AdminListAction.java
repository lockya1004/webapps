	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(hs.getAttribute("loginUser") == null) {
			...
			if(keyword=="") {
				...
				list=pdao.listProd(vo.getAdm_id(), vo.getAdm_type(),keyword,page,per_cnt);
				listCount=pdao.get_prod_cnt(vo.getAdm_id(), vo.getAdm_type(),keyword);
			}
			
			//페이지 목록 처리를 위한 계산 작업
	        // 1.전페 페이지 수 계산( (총 게시물 수/페이지 문서 수)+1)
	        int maxPage=(int)((double)listCount/per_cnt)+1;
